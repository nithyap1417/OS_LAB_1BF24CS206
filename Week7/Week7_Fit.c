...
int blockSize[m];
int block1[m], block2[m], block3[m]; printf("Enter sizes of %d memory blocks:\n", m); for (int i = 0; i < m; i++)
{
scanf("%d", &blockSize[i]); block1[i] = blockSize[i]; block2[i] = blockSize[i]; block3[i] = blockSize[i];
}
...
firstFit(block1, m, processSize, n); bestFit(block2, m, processSize, n); worstFit(block3, m, processSize, n); return 0;
}
